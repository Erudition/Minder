...
updateViewSettings : Profile -> Shared.Model.Model -> ViewSettings
...
init shared () =
    let
        ...
        initialSettings =
            updateViewSettings shared.replica shared
            ...
            layout [ width fill, height fill ] <|
                column [ width fill, height fill ]
                    ...
                    , row
                        ...
                        [ Element.html <| svgExperiment model profile projectLayers env ]
                    , row [ width fill, height (px 30), Background.color (Element.rgb 0.5 0.5 0.5) ]
                        [ el [ centerX ] <|
                            Element.text
                                (multiline
                                    [ [ "Showing"
                                      , Period.length model.settings.flowRenderPeriod
                                            |> HumanDuration.breakdownDH
                                            |> HumanDuration.trim
                                            |> HumanDuration.abbreviatedSpaced
                                      , "(from"
                                      , Period.start model.settings.flowRenderPeriod
                                            |> HumanMoment.describeVsNow env.timeZone env.time
                                      , "through"
                                      , Period.end model.settings.flowRenderPeriod
                                            |> HumanMoment.describeVsNow env.timeZone env.time
                                      , ")"
                                      ]
                                    ]
...
-- svgExperiment : Model -> Profile -> ProjectLayers -> Shared.Model.Model -> H.Html Msg
...
timeLabel : Shared.Model.Model -> Moment -> Shape msg
...
blobToShape : Model -> Shared.Model.Model -> FlowBlob -> Shape Msg
...
blobToPoints : ViewSettings -> Shared.Model.Model -> FlowBlob -> { shell : Polygon, bestTextArea : ( Point, Point ), startCapTL : Point, endCapTL : Point, midHeight : Float, reversed : ( Bool, Bool ) }
...
historyBlobs : Shared.Model.Model -> Profile -> ProjectLayers -> Period -> List FlowBlob
...
-}
dragOffsetDur : Model -> Float -> ( Bool, Bool ) -> Point -> Duration
...
dayString : Shared.Model.Model -> Moment -> String
dayString env moment =
    Calendar.toStandardString (HumanMoment.extractDate env.timeZone moment)


makeHistoryBlob : Shared.Model.Model -> Profile -> ProjectLayers -> Period -> HistorySession -> FlowBlob
...
update shared msg state =
    ...
    case msg of
        ChangeTimeWindow newStart newFinish ->
            ...
            in
            ( { state | settings = withNewPeriodToRender }, Effect.none )
        ...
        Wheel wheelEvent ->
            ...
            in
            ( { state | settings = newSettings }, Effect.none )
        ...
        WidgetMsg widgetMsg ->
            ...
            , Effect.sendCmd (Cmd.map WidgetMsg widgetOutCmds)
            )
        ...
        PointerMove ( x, y ) ->
            ...
            in
            ( { state | pointer = newPointer }, Effect.none )
        ...
        ResetCoordinates width height ->
            ...
            , Effect.sendCmd (Cmd.map WidgetMsg widget1init)
            )
