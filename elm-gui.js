...
var author$project$Incubator$Todoist$emptyCache = {P: elm_community$intdict$IntDict$empty, as: '*', bz: _List_Nil, T: elm_community$intdict$IntDict$empty};
...
var elm_community$intdict$IntDict$lcp = F2(
	function (x, y) {
		...
		return {aE: branchingBit, S: prefixBits};
	});
...
var author$project$Incubator$Todoist$Cache = F4(
	function (nextSync, items, projects, pendingCommands) {
		return {P: items, as: nextSync, bz: pendingCommands, T: projects};
	});
...
var author$project$Incubator$Todoist$Response = F5(
	function (sync_token, sync_status, full_sync, items, projects) {
		return {cI: full_sync, P: items, T: projects, ew: sync_status, dz: sync_token};
	});
...
var author$project$Integrations$Todoist$findActivityProjects = F2(
...
var author$project$Integrations$Todoist$detectActivityProjects = F3(
	function (maybeParent, app, cache) {
		if (maybeParent.$ === 1) {
			...
			var newActivityLookupTable = A2(author$project$Integrations$Todoist$findActivityProjects, validActivityProjects, activities);
			return A2(elm_community$intdict$IntDict$union, newActivityLookupTable, app.cc.bI);
		}
	});
...
var author$project$Integrations$Todoist$handle = F2(
	function (msg, app) {
		...
		if (!_n0.$) {
			...
			var newCache = _n1.a;
			var changes = _n1.b;
			var maybeParent = A2(author$project$Integrations$Todoist$tryGetTimetrackParentProject, app.cc, newCache);
			var projectToActivityMapping = A3(author$project$Integrations$Todoist$detectActivityProjects, maybeParent, app, newCache);
			...
			return _Utils_Tuple2(
				_Utils_update(
					app,
					{
						...
						cc: {bI: projectToActivityMapping, bK: newCache, b$: maybeParent}
					}),
...
var author$project$Main$Model = F3(
	function (viewState, appData, environment) {
		return {O: appData, V: environment, bg: viewState};
	});
				...
				if (!_n14.b.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								O: A2(author$project$AppData$saveError, appData, problemText)
							}),
			...
			elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						...
						author$project$Main$appDataToJson(newModel.O)),
						cmds
					])));
	});
