...
**Honest status:** The Project layer may be partially redundant with Minder's *Cares* system (see [cares.md](cares.md)). Many real-world groupings that seem like Projects turn out to be Cares — entities or concerns you "take care of" (your car, your job, your Spanish skills), where the tasks are maintenance of that entity. The Project layer is retained because (a) the type system already supports it cleanly, (b) there may be legitimate groupings that are about coordinated effort rather than entity maintenance, (c) removing a layer prematurely is riskier than leaving a rarely-used one in place, and (d) the Project layer serves as a **migration staging area** — when importing data from other apps (e.g. Todoist), the imported tree structure can be parked as Projects while the system determines where the Assignable boundary belongs and which branches are really Cares.
