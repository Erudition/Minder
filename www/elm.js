...
var author$project$AppData$fromScratch = {errors: _List_Nil, tasks: _List_Nil, uid: 0};
...
var author$project$AppData$AppData = F3(
	function (uid, errors, tasks) {
		return {errors: errors, tasks: tasks, uid: uid};
	});
...
var zwilias$json_decode_exploration$Json$Decode$Exploration$map3 = F4(
...
var author$project$AppData$decodeAppData = A4(
	zwilias$json_decode_exploration$Json$Decode$Exploration$map3,
	...
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'errors',
				A2(elm$json$Json$Encode$list, elm$json$Json$Encode$string, record.errors))
			]));
};
...
var author$project$Main$updateWithStorage = F2(
	function (msg, model) {
		var _n0 = A2(author$project$Main$updateWithTime, msg, model);
		var newModel = _n0.a;
		var cmds = _n0.b;
		return _Utils_Tuple2(
			newModel,
			elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						...
						author$project$Main$appDataToJson(model.appData)),
						cmds
					])));
	});
...
var author$project$Task$TaskMoment$describeTaskMoment = F2(
	function (now, target) {
		switch (target.$) {
			...
			case 'LocalDate':
				...
				return 'in ' + (elm$core$String$fromInt(
					A3(
						...
						date,
						A2(justinmimbs$date$Date$fromPosix, author$project$Task$TaskMoment$userTimeZonePlaceholder, now))) + ' days');
			case 'Localized':
				var moment = target.a;
				return A2(
			...
			default:
				var moment = target.a;
				return A2(sporto$time_distance$Time$Distance$inWords, now, moment);
		}
	});
				...
				[
					...
					_List_fromArray(
						[
							...
							app.tasks),
							A3(rtfeldman$elm_css$Html$Styled$Lazy$lazy2, author$project$TaskList$viewControls, filters, app.tasks)
						])),
					A2(
					rtfeldman$elm_css$Html$Styled$section,
					_List_fromArray(
...
var author$project$Main$main = elm$browser$Browser$application(
	{init: author$project$Main$init, onUrlChange: author$project$Main$NewUrl, onUrlRequest: author$project$Main$Link, subscriptions: author$project$Main$subscriptions, update: author$project$Main$updateWithStorage, view: author$project$Main$view});
