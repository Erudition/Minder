...
- **Assignments are timing-agnostic**: Assigning ≠ scheduling. An Assignment is a commitment to do something, not a calendar entry. Timing fields (deadlines, relevance windows) are all optional. An assigned-but-unscheduled task is the normal, expected state. Scheduling is a separate system that consumes task layers as input.
